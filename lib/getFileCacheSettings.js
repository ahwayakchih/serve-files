...
function getCachedFileInfo (cfg, req, res, filePath, callback) {
	...
	fs.open(filePath, 'r', function (err, fd) {
		...
		cachedFile.until = cfg.cacheTimeInSeconds === 0 ? 0 : Date.now() + (cfg.cacheTimeInSeconds * 1000);

		if (err) {
			// Enforce cache timeout on errors, to prevent keeping them forever.
			cachedFile.until = Math.min(cachedFile.until, 10000);
		...
		getFileInfo(cfg, req, res, filePath, function (cfg, req, res, filePath, fileStats) {
